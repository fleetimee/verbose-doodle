  ...
  return (
    <>
      {items.map((item) => {
        ...
        const srcSet =
          item.imageWebp1x && item.imageWebp2x
            ? `${item.imageWebp1x} 1x, ${item.imageWebp2x} 2x`
            : undefined;
        ...
        return (
          ...
          >
            ...
            {!isLoaded && (
              <Skeleton className="absolute inset-0 h-14 w-14 rounded-full border-2 border-white" />
            )}
            <img
              ...
              width={item.width || 56}
              src={item.imageWebp1x || item.image}
              srcSet={srcSet}
              loading="lazy"
              decoding="async"
              alt={item.name}
